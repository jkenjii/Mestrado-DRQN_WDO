...
class DQNAgent(Agent):    
    ...
    def __init__(self,
        ...
        self.window_steps_per_episode = 128 #trace_length, tamanho de cada episodio 128 
        self.policy_network = policy_network or self._build_policy_network()
        
        self.target_network = policy_network or self._build_policy_network()
    ...
    def _build_policy_network(self):
        ...
        print(inputs)
        #a = tf.keras.layers.Conv1D(filters=64, kernel_size=3,dilation_rate=2,  activation='relu', kernel_initializer='HeNormal')(inputs)
        #a = tf.keras.layers.Dropout(0.2)(a)
        #a = tf.keras.layers.Conv1D(filters=64, kernel_size=2,dilation_rate=4,  activation='relu', kernel_initializer='HeNormal')(a)
        #a = tf.keras.layers.Dropout(0.2)(a)
        #a = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(11, activation='relu'))(inputs)
        #a = tf.keras.layers.Reshape((a.shape[1], a.shape[2], 1))(a)
        a = tf.keras.layers.Reshape((inputs.shape[1], inputs.shape[2], 1))(inputs)
        a = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), dilation_rate=2, activation='relu', kernel_initializer='HeNormal')(a)
        #a = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3),  padding="same",activation='relu', kernel_initializer='HeNormal')(inputs)
        #print(a)
        #a = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3),strides=(2, 2), activation='relu', kernel_initializer='HeNormal')(a)
        #a = tf.keras.layers.MaxPooling1D(pool_size=2)(a)
        #print(a)
        #a = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), activation='relu', kernel_initializer='HeNormal')(a)
        a = tf.keras.layers.Conv2D(filters=32, kernel_size=(2, 2),  dilation_rate=4, activation='relu', kernel_initializer='HeNormal')(a)
        #a = tf.keras.layers.Conv2D(filters=1, kernel_size=(1, 1), activation='relu', kernel_initializer='HeNormal')(a)
        #a = tf.keras.layers.Conv2D(filters=32, kernel_size=(2, 2),  padding="same",activation='relu', kernel_initializer='HeNormal')(a)
        #a = tf.keras.layers.Conv2D(filters=32, kernel_size=(2, 2),strides=(2, 2), activation='relu', kernel_initializer='HeNormal')(a)
        #res = tf.keras.layers.Reshape((a.shape[1], a.shape[2]*a.shape[3]))(a)
        res = tf.keras.layers.Reshape((a.shape[1]*a.shape[2],a.shape[3]))(a)
        #flat = tf.keras.layers.Flatten()(a)
        #dense = tf.keras.layers.Dense(156, activation='relu')(flat)
        #print(a)
        #a = tf.keras.layers.MaxPooling1D(pool_size=2)(a)
        #print(a)
        ...
        lstm  = tf.keras.layers.LSTM(156)(res)
        ...
        #print(lstm)
        ult_dense = tf.keras.layers.Dense(32, activation='elu')(lstm)
    ...
    def save(self, path: str, **kwargs):
        ...
        episode = str(episode)
        if episode:
            filename = "policy_network__" + self.id[:7] + "__" + datetime.now().strftime("%Y%m%d_%H%M%S") + "_" + episode + ".hdf5"
        else:
            filename = "policy_network__" + self.id[:7] + "__" + datetime.now().strftime("%Y%m%d_%H%M%S") + "_" + episode + ".hdf5"
    ...
    def _apply_gradient_descent(self, memory: ReplayMemory, batch_size: int, learning_rate: float, discount_factor: float, trace_length):
        #https://github.com/chagmgang/tf2.0_reinforcement_learning/blob/master/rainbow/dqn.py
        ...
        #    elf.train_op = optimizer.apply_gradients(zip(grads, vs))
        #else:
    ...
    def _apply_gradient_descent_DDQN(self, memory: ReplayMemory, batch_size: int, learning_rate: float, discount_factor, trace_length):
        #https://github.com/chagmgang/tf2.0_reinforcement_learning/blob/master/rainbow/double_dqn.py
        #https://alexandervandekleut.github.io/deep-q-learning/
        #clipnorm
        optimizer = tf.keras.optimizers.Adam(lr=learning_rate, clipnorm=1.0)
        loss = tf.keras.losses.Huber()
    ...
    def train(self,
        ...
        learning_rate: float = kwargs.get('learning_rate', 0.00025)
        eps_start: float = kwargs.get('eps_start', 0.9)
        eps_end: float = kwargs.get('eps_end', 0.05)
        eps_decay_steps: int = kwargs.get('eps_decay_steps', 200)
        update_target_every: int = kwargs.get('update_target_every',2338)#2412)#1512) #700 8000 1512
        memory_capacity: int = kwargs.get('memory_capacity', 23380) #4514000 para 1 ano 15120
        ...
        #decay = 0.9977 #1100 com fim em 1000 27/10/2021
        #decay = 0.999 #3000 para 2 meses 28/10/2021
        decay = 0.99933 #4500 para 3 meses 04/12/2021
        #decay = 0.99855 #2250 para dados de 30 min 16/02/2022
        #decay = 0.992 #375 14/07
        #decay = 0.976 #100 25/07
        #decay  = 0.996
        #decay = 0.9961 #750 15/07
        #decay = 0.994 #500 15/07
        #decay = 0.9935 #450 15/07
        #decay = 0.96 #75 13/07/2021 deu um retorno de 382 em 1 semana
        #decay = 0.975 #120 13/07/2021
        min_epsilon = 0.1
        ...
        for n in range(n_episodes):
            ...
            while not done:
                ...
                #if (len(memory) > self.window_steps_per_episode) and (steps_done % 36 == 0): oliginal 14/12/2021
                if (len(memory) > self.window_steps_per_episode*10) and (steps_done % 4 == 0):
                    #print('ESTA TREINANDO')
